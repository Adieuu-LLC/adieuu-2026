...
For AWS Fargate, build for the task CPU architecture (`linux/arm64` for the default ARM64/Graviton config):

```bash
docker build --platform linux/arm64 -f apps/api/Dockerfile -t adieuu-api:local .
docker build --platform linux/arm64 -f apps/chat/Dockerfile -t adieuu-chat:local .
```
...
| **Chat** | `node:26-trixie-slim` | `node dist/index.mjs` (esbuild bundle + external `node_modules`) | `9001` (`CHAT_PORT`) |
...
- **API builder** uses **Node 25** (Bookworm slim) + pnpm and copies the `bun` binary from `oven/bun:1.2` so `bun build` matches local behavior.
- **Chat production bundle** uses `esbuild` (`--target=node26`) with `--packages=external` so application code is one ESM file while native modules such as `uWebSockets.js` stay in `node_modules`. This avoids Node's ESM limitation on directory imports from plain `tsc` output.
- The chat Dockerfile requires **Node 26** (Trixie slim) for its build and runtime stages; the API builder uses Node 25 (Bookworm slim) but the API **runtime** image is Bun-only.
