...
#
# Two origins:
#   1. s3-media-processed: serves processed/ prefix (public GET, cached)
#   2. s3-media-uploads:   accepts uploads/ prefix (signed URL PUT, uncached)
#
# The upload behavior is gated by a trusted key group: only requests with a
# valid CloudFront signed URL (generated by the API) are forwarded to S3.
...
resource "aws_cloudfront_distribution" "media" {
  ...
  # Origin 1: processed media (read-only, cached)
  origin {
    domain_name              = aws_s3_bucket.media[0].bucket_regional_domain_name
    origin_id                = "s3-media-processed"
    origin_access_control_id = aws_cloudfront_origin_access_control.media[0].id
    origin_path              = "/processed"
  }

  # Origin 2: upload target (write, uncached) — same bucket, no origin_path
  origin {
    domain_name              = aws_s3_bucket.media[0].bucket_regional_domain_name
    origin_id                = "s3-media-uploads"
    origin_access_control_id = aws_cloudfront_origin_access_control.media[0].id
  }

  # Upload behavior: uploads/* path, PUT allowed, requires signed URL
  dynamic "ordered_cache_behavior" {
    ...
    content {
      ...
      target_origin_id         = "s3-media-uploads"
      compress                 = false
  ...
  # Default behavior: serve processed media (public GET, cached)
  default_cache_behavior {
    allowed_methods          = ["GET", "HEAD", "OPTIONS"]
    cached_methods           = ["GET", "HEAD"]
    target_origin_id         = "s3-media-processed"
    compress                 = true
