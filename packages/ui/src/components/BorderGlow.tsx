/**
 * Pointer-tracking border glow effect with optional animated intro sweep.
 *
 * Adapted from react-bits (MIT + Commons Clause)
 * @see https://reactbits.dev/animations/border-glow
 */

import { useRef, useCallback, useEffect, type ReactNode, type PointerEvent as ReactPointerEvent } from 'react';
...
function buildGradientVars(colors: string[]): Record<string, string> {
  const vars: Record<string, string> = {};
  for (let i = 0; i < 7; i++) {
    const idx = COLOR_MAP[i] ?? 0;
    const c = colors[Math.min(idx, colors.length - 1)] ?? colors[0];
