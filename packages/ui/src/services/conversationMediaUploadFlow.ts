...
export async function uploadModerationScanCopy(
  ...
  for (const part of parts) {
    ...
    const { scanMediaId, uploadUrl: scanUrl, uploadFields, uploadHeaders } = scanRes.data;
    ...
    if (uploadHeaders) {
      ...
      throw new Error('Scan upload response missing both uploadHeaders and uploadFields');
    }
